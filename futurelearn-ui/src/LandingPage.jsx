...
export default function LandingPage() {
  ...
  const [modal,         setModal]         = useState(null);   // clé modale ouverte
  ...
  // ── Compteur animé avec IntersectionObserver ──────────────────────────────
  useEffect(() => {
    ...
    <>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body, #landing-root { font-family: 'Montserrat', sans-serif; line-height: 1.6; color: var(--text-dark); background: var(--light-bg); }
        ...
        .li-navbar.scrolled { box-shadow: 0 4px 20px rgba(0,0,0,0.15); }
        ...
        .li-nav-links a { color: var(--text-dark); font-weight: 500; font-size: 1rem; padding: 5px 0; position: relative; }
        .li-nav-links a:hover { color: var(--primary); text-decoration: none; }
        .li-nav-links a::after { content:''; position:absolute; left:0; bottom:-5px; width:0; height:2px; background:var(--primary); transition:width .3s; }
        .li-nav-links a:hover::after { width: 100%; }
        .li-nav-btn { border: 2px solid var(--primary); padding: 5px 15px; border-radius: 5px; font-weight: bold; color: var(--primary) !important; }
        .li-nav-btn:hover { background: var(--primary); color: white !important; text-decoration: none; }
        .li-menu-toggle { display: none; background: none; border: none; font-size: 1.8rem; color: var(--primary); cursor: pointer; }
        ...
        .li-hero video {
          ...
          object-fit: cover; filter: blur(5px) brightness(0.6); z-index: -1;
        }
        ...
        .li-hero-content p { font-size: 1.8rem; font-weight: 300; text-shadow: 1px 1px 5px rgba(0,0,0,0.7); margin-bottom: 40px; }
        ...
        .li-btn-hero {
          ...
          text-shadow: 0 2px 4px rgba(0,0,0,0.3); min-width: 240px; border: none;
          box-shadow: 0 10px 20px rgba(0,0,0,0.2); transition: all 0.3s cubic-bezier(.175,.885,.32,1.275); cursor: pointer;
        }
        .li-btn-hero:hover { transform: translateY(-5px) scale(1.05); box-shadow: 0 15px 30px rgba(0,0,0,0.4); text-decoration: none; }
        .li-btn-student { background: linear-gradient(135deg,#8b5cf6,#6d28d9); border: 2px solid rgba(255,255,255,0.3); }
        .li-btn-student:hover { background: linear-gradient(135deg,#a78bfa,#7c3aed); }
        .li-btn-teacher { background: linear-gradient(135deg,#1e1b4b,#312e81); border: 2px solid rgba(255,255,255,0.1); }
        .li-btn-teacher:hover { background: linear-gradient(135deg,#312e81,#4338ca); }
        ...
        .li-light { background: var(--light-bg); }
        .li-dark { background: var(--primary-dark); color: white; }
        ...
        .li-dark .li-sec-header h2 { color: white; }
        .li-sec-header p { font-size: 1.3rem; color: var(--text-light); max-width: 800px; margin: 0 auto; }
        .li-dark .li-sec-header p { color: rgba(255,255,255,0.8); }

        /* ── Fonctionnalités ── */
        .li-course-grid { display: grid; grid-template-columns: repeat(auto-fit,minmax(300px,1fr)); gap: 30px; text-align: left; }
        .li-course-card { background: var(--card-bg); border-radius: var(--radius); box-shadow: var(--shadow); padding: 30px; transition: transform .3s, box-shadow .3s; }
        .li-course-card:hover { transform: translateY(-8px); box-shadow: 0 10px 30px rgba(0,0,0,0.1); }
        .li-course-card h3 { font-size: 1.8rem; font-weight: bold; color: var(--primary); margin-bottom: 25px; }
        ...
        .li-course-card ul li i { color: #4CAF50; position: absolute; left: 0; top: 3px; font-size: 1.1em; }
        .li-course-card .li-btn-primary { width: 100%; padding: 10px 15px; border-radius: 8px; }

        /* ── Boutons généraux ── */
        .li-btn-primary { display: inline-block; padding: 12px 25px; border-radius: 50px; font-weight: bold; font-size: 1rem; background: var(--primary); color: white; border: none; cursor: pointer; transition: all .3s; }
        .li-btn-primary:hover { background: var(--primary-dark); color: white; text-decoration: none; }
        .li-btn-secondary { display: inline-block; padding: 12px 25px; border-radius: 50px; font-weight: bold; font-size: 1rem; background: transparent; border: 2px solid var(--primary); color: var(--primary); cursor: pointer; transition: all .3s; }
        .li-btn-secondary:hover { background: var(--primary); color: white; text-decoration: none; }

        /* ── Pourquoi nous ── */
        .li-tile-grid { display: grid; grid-template-columns: repeat(auto-fit,minmax(280px,1fr)); gap: 30px; }
        .li-tile-card { background: var(--primary); color: white; border-radius: var(--radius); padding: 40px 30px; display: flex; flex-direction: column; align-items: center; transition: transform .3s, box-shadow .3s; }
        .li-tile-card:hover { transform: translateY(-8px); box-shadow: 0 10px 30px rgba(0,0,0,0.1); }
        .li-tile-card img { width: 80px; height: 80px; margin-bottom: 25px; filter: invert(1); }
        .li-tile-card h3 { font-size: 2rem; font-weight: bold; color: white; margin-bottom: 20px; text-align: center; }
        .li-tile-card p { font-size: 1.1rem; color: rgba(255,255,255,0.9); margin-bottom: 30px; text-align: center; }
        .li-tile-card .li-btn-secondary { border-color: white; color: white; border-radius: 8px; padding: 10px 20px; }
        .li-tile-card .li-btn-secondary:hover { background: white; color: var(--primary-dark); }

        /* ── Comment ça marche ── */
        .li-process-grid { display: grid; grid-template-columns: repeat(auto-fit,minmax(300px,1fr)); gap: 30px; margin-top: 40px; }
        .li-process-step { background: var(--card-bg); border-radius: var(--radius); box-shadow: var(--shadow); padding: 30px; text-align: center; transition: transform .3s, box-shadow .3s; }
        .li-process-step:hover { transform: translateY(-8px); box-shadow: 0 10px 30px rgba(0,0,0,0.1); }
        .li-icon-circle { width: 80px; height: 80px; background: var(--primary); border-radius: 50%; display: flex; justify-content: center; align-items: center; margin: 0 auto 25px; color: white; font-size: 2.5rem; }
        .li-process-step h3 { font-size: 1.8rem; font-weight: bold; color: var(--primary); margin-bottom: 15px; }
        .li-process-step p { color: var(--text-light); font-size: 1.1rem; }

        /* ── Compteur ── */
        .li-counter-wrap { margin-bottom: 60px; color: white; }
        .li-counter-text { font-size: 1.5rem; font-weight: 300; margin-bottom: 10px; }
        .li-counter-num { font-size: 5rem; font-weight: 900; color: white; text-shadow: 2px 2px 8px rgba(0,0,0,0.3); display: inline-block; }
        ...
        .li-testimonial-card { min-width: 100%; padding: 40px; background: rgba(255,255,255,0.1); border-radius: var(--radius); color: white; text-align: center; }
        .li-testimonial-card img { width: 100px; height: 100px; border-radius: 50%; object-fit: cover; margin-bottom: 20px; border: 3px solid white; }
        .li-testimonial-card h3 { font-size: 1.8rem; font-weight: bold; margin-bottom: 5px; }
        .li-testimonial-card .li-role { font-size: 1.1rem; color: rgba(255,255,255,0.7); margin-bottom: 25px; }
        .li-testimonial-card blockquote { font-size: 1.3rem; line-height: 1.8; color: rgba(255,255,255,0.9); font-style: italic; padding: 0 20px; }
        .li-carousel-nav { position: absolute; top: 50%; transform: translateY(-50%); background: rgba(255,255,255,0.2); border: none; color: white; font-size: 1.5rem; width: 45px; height: 45px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background .3s; z-index: 10; }
        .li-carousel-nav:hover { background: rgba(255,255,255,0.4); }
        ...
        .li-contact-info p i { color: var(--primary); margin-right: 15px; font-size: 1.5rem; }
        .li-contact-form { display: flex; flex-direction: column; gap: 20px; background: var(--card-bg); border-radius: var(--radius); box-shadow: var(--shadow); padding: 40px; }
        .li-contact-form input, .li-contact-form textarea { padding: 15px; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem; font-family: 'Montserrat',sans-serif; resize: vertical; }
        .li-contact-form input:focus, .li-contact-form textarea:focus { border-color: var(--primary); outline: none; box-shadow: 0 0 0 3px rgba(106,55,183,0.1); }
        .li-contact-form .li-btn-primary { padding: 15px 25px; font-size: 1.1rem; border-radius: 8px; }

        /* ── Footer ── */
        .li-footer { background: #222; color: white; padding: 40px 20px; text-align: center; font-size: 0.9rem; }
        .li-footer-inner { max-width: 1200px; margin: 0 auto; }
        .li-footer p { margin-bottom: 15px; }
        .li-footer-nav a { color: white; margin: 0 15px; }
        .li-footer-nav a:hover { text-decoration: underline; }

        /* ── Modale ── */
        .li-modal-overlay { display: none; position: fixed; z-index: 3000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(255,255,255,0.3); backdrop-filter: blur(10px); animation: fadeIn .3s; }
        .li-modal-overlay.open { display: flex; align-items: center; justify-content: center; }
        .li-modal-box { background: white; padding: 40px; border-radius: 20px; width: 90%; max-width: 600px; border-left: 15px solid var(--primary); position: relative; box-shadow: 0 20px 50px rgba(0,0,0,0.5); }
        .li-modal-close { position: absolute; right: 25px; top: 15px; font-size: 35px; cursor: pointer; color: var(--primary); background: none; border: none; }
        .li-modal-box h2 { color: var(--primary); margin-bottom: 20px; font-weight: 700; }
        .li-modal-box p { line-height: 1.6; color: #444; font-size: 1.1rem; }
        @keyframes fadeIn { from{opacity:0} to{opacity:1} }

        /* ── Responsive ── */
        @media(max-width:992px){
          .li-nav-links { display:none; flex-direction:column; width:100%; background:white; position:absolute; top:70px; left:0; box-shadow:0 5px 10px rgba(0,0,0,0.05); padding-bottom:10px; }
          .li-nav-links.open { display:flex; }
          .li-nav-links li { margin:0; text-align:center; }
          .li-nav-links a { display:block; padding:15px 20px; border-bottom:1px solid #eee; }
      ...
      <div id="landing-root">
        ...
        {/* ══ POURQUOI NOUS ══ */}
        <section id="why-us" className="li-section">
          <div className="li-inner">
            <div className="li-sec-header">
        ...
        {/* ══ TÉMOIGNAGES ══ */}
        <section id="testimonials" className="li-section li-dark">
          <div className="li-inner">
            <div className="li-sec-header">
